...
#+begin_src noweb :tangle ../dingehaufen/conf.d/abbreviations.fish :exports none :noweb yes
# nikola
<<nikola>>
...
** PDF Shrink
Uses Ghostscript to reduce images to "epub" level.

#+begin_src fish :noweb-ref pdf-shrink
...
- This is also in my {{% lancelot title="tealdeer" %}}tealdeer-pages{{% /lancelot %}} post.

* Emacs
** Server Status

  #+begin_src fish :noweb-ref emacs-server-status
  abbr --add -- emacsstatus "systemctl --user status emacs"
  #+end_src

** Restart Server

  #+begin_src fish :noweb-ref emacs-server-restart
...
#+begin_src fish :noweb-ref enumerate-files
abbr --add -- lsn "ls --color | cat -n"
#+end_src

** Files By Size

List one file per line (=-1=) sorted by size (=-S) showing human-readable sizes (=-sh=).

#+begin_src fish :noweb-ref files-by-size
abbr --add -- lS "ls -sSh1"
#+end_src

* Byobu (screen)
** Re-attach
I've been using =byobu -S <name>= to create and =byobu -rd <name>= to create named screen sessions, but I sometimes forget if I've already created a session with the same name or not and either create another with the same name, get an error message telling me there's no session with that name, or use =byobu -ls= to check on existing screen sessions first. This seems like extra work, and it turns out that you can have =screen= do the checking for you by capitalizing the =-R= option.
...
But, I sometimes forget the short-codes, especially when you have to use lowercase for one option and uppercase for another (why is re-attach capitalized and not detach?), so I'm making an abbreviation. We'll see if I can remember to use it.

#+begin_src fish :noweb-ref byobu-screen
abbr --add -- bs "byobu -dR"
#+end_src

** List

#+begin_src fish :noweb-ref byobu-list
