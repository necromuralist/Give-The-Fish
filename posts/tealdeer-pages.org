...
.. type: text
.. status: 
.. updated: 2026-09-23 16:31:01 UTC-07:00
...
#+begin_notecard
**Note:** February 18, 2026 - tealdeer via apt is/was out of date. They changed the URL used for updating the cache (see https://github.com/tealdeer-rs/tealdeer/issues/459) so versions older than 1.8 throw an error (apt currently installs 1.7.2-1). To get 1.8 I used [[https://tealdeer-rs.github.io/tealdeer/installing.html#through-cargo-install][the Cargo Installation Instructions]] (which necessitated [[https://doc.rust-lang.org/cargo/getting-started/installation.html][installing cargo and rust]]...).
#+end_notecard
...
<<gs-black-and-white-no-images-pdf>>
...
#+begin_src markdown :noweb-ref gs-black-and-white-no-images-pdf
...
#+begin_src markdown :noweb-ref gs-to-grayscale
- Convert color to grayscale

`gs -sProcessColorModel=DeviceGray -sColorConversionStrategy=Gray -sOutputFile={{output-file-name}} {{input-file-name}}`
#+end_src

Source: [[https://stackoverflow.com/a/20133915][Stack Overflow]] - /Convert PDF to grayscale without rasterization?/

*** Resize (to Letter)

Evince no longer has the "Scale to Printable Area" option when printing PDFs, so this is something to use to get the PDF to letter-sized before printing. (from [[https://stackoverflow.com/a/7507511][Stack Overflow: Resizing a PDF Using Ghostscript]])

#+begin_src markdown :noweb-ref gs-to-letter-size
...
#+begin_src noweb :tangle ../dingehaufen/tealdeer/pages/pdftk.patch.md :exports none
<<pdftk-export-bookmarks>>

<<pdftk-import-bookmarks>>
#+end_src
...
Taken from [[https://www.pdflabs.com/blog/export-and-import-pdf-bookmarks/][PDFLabs Export and Import PDF Bookmarks]]. 

#+begin_src markdown :noweb-ref pdftk-export-bookmarks
...
#+begin_src markdown :noweb-ref pdftk-import-bookmarks
...
[[https://exiftool.org/][exiftool]] is a metadata editor. It handles many file types but I used it for PDFs.

#+begin_src noweb :tangle ../dingehaufen/tealdeer/pages/exiftool.patch.md :exports none
<<exiftool-title>>
#+end_src

*** Title

#+begin_src markdown :noweb-ref exiftool-title
...
I use [[https://git-scm.com/book/en/v2/Git-Basics-Git-Aliases][git aliases]] (and also [[https://kennethreitz.org/software/legit][legit]]) to remember git things for me as much as possible, but this is for things that are sort of as simple as they should be but I keep forgetting about them anyway.

#+begin_src noweb :tangle ../dingehaufen/tealdeer/pages/git.patch.md :exports none
<<git-checkout-file>>
#+end_src

*** Checkout a File From Another Branch

#+begin_src fish markdown :noweb-ref git-checkout-file
- Checkout a file from another branch
...
#+begin_src noweb :tangle ../dingehaufen/tealdeer/pages/keychron.page.md :exports none
# keychron K12

> Keychron K12 Keyboard

<<keychron-bluetooth-pairing>>
#+end_src

*** Bluetooth Pairing

#+begin_src markdown :noweb-ref keychron-bluetooth-pairing
